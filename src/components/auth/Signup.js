      ...
      <div className="full-page-container" style={{
        ...
        <div className="left-panel" style={{
          ...
          <div style={{
            ...
            width: '100%',
            height: '100%',
            background: `repeating-linear-gradient(45deg, ${colors.accentGold}08, ${colors.accentGold}08 10px, transparent 10px, transparent 20px)`,
            opacity: 0.8,
            zIndex: 0,
          }}></div>
          <img src="/authimg.png" alt="Authentication" style={{
            width: '100%',
            height: '100%',
            objectFit: 'cover',
        ...
        <div className="right-panel" style={{
          ...
          <form onSubmit={handleSubmit} style={{
            ...
            <div style={{ position: 'relative', width: '100%' }}>
              <label htmlFor="email" style={{ ...labelBaseStyle, ...(shouldLabelFloat(isEmailFocused, email) ? labelFloatStyle : {}) }}>Email Address</label>
              <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setIsEmailFocused(true)} onBlur={() => setIsEmailFocused(false)} style={{ ...inputBaseStyle, ...(isEmailFocused ? inputFocusStyle : {}) }} />
            </div>

            <div style={{ position: 'relative', width: '100%' }}>
              <label htmlFor="username" style={{ ...labelBaseStyle, ...(shouldLabelFloat(isUsernameFocused, username) ? labelFloatStyle : {}) }}>Username</label>
              <input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} onFocus={() => setIsUsernameFocused(true)} onBlur={() => setIsUsernameFocused(false)} style={{ ...inputBaseStyle, ...(isUsernameFocused ? inputFocusStyle : {}) }} />
            </div>

            <div style={{ position: 'relative', width: '100%' }}>
              <label htmlFor="password" style={{ ...labelBaseStyle, ...(shouldLabelFloat(isPasswordFocused, password) ? labelFloatStyle : {}) }}>Password</label>
              <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} onFocus={() => setIsPasswordFocused(true)} onBlur={() => setIsPasswordFocused(false)} style={{ ...inputBaseStyle, ...(isPasswordFocused ? inputFocusStyle : {}) }} />
            </div>

            <div style={{ position: 'relative', width: '100%' }}>
              <label htmlFor="confirmPassword" style={{ ...labelBaseStyle, ...(shouldLabelFloat(isConfirmPasswordFocused, confirmPassword) ? labelFloatStyle : {}) }}>Confirm Password</label>
              <input id="confirmPassword" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} onFocus={() => setIsConfirmPasswordFocused(true)} onBlur={() => setIsConfirmPasswordFocused(false)} style={{ ...inputBaseStyle, ...(isConfirmPasswordFocused ? inputFocusStyle : {}) }} />
            </div>
            ...
            <button type="button" onClick={handleGoogleSignUp} style={googleButtonBaseStyle}
            ...
            <div style={{ marginTop: '30px', fontSize: '0.98em', color: colors.textSecondary, textAlign: 'center' }}>
              Already have an account?{" "}
              <button type="button" onClick={() => router.push('/auth/signin')}
                style={{ color: colors.accentGold, textDecoration: 'none', fontWeight: '600', transition: 'color 0.2s ease', background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}
